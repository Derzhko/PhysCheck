...
public class Testm2Activity extends Activity implements View.OnClickListener{
    ...
    TextView tvtrue, tvfalse, tv1, tv2, tv3, tventer;
    Random rndposition = new Random();
    Random rndindex = new Random();
    Random rndother = new Random();
    String othernames[] = new String [names.length];
    int position;
    int index;
    int itrue, ifalse;
    TextView arrTv [];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        arrTv = new TextView[]{tv1, tv2, tv3};
        change();
    }

    private void change(){
        if (popitki > 0){
            position = rndposition.nextInt(3);
            ...
            tventer.setText(formuls[index]);
            arrTv[position].setText(names[index]);
            for (int i = 0; i < arrTv.length; i++) {
                if (arrTv[i] != arrTv[position])
                    arrTv[i].setText(othernames[rndother.nextInt(names.length)]);
            }
